...
*Setup instructions for a Gitian build of MarteX Core using a Debian VM or physical system.*
...
Clone the git repositories for MarteX Core and Gitian.
...
in the MarteX Core repository under 'Fetch and create inputs' to install sources which require
...
To build MarteX Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the MarteX Core repository.
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/martex-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/martex-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/martex-osx-build.assert
```
