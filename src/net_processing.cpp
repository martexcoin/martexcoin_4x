...
bool found_304, found_3041, found_3042, found_3051, found_3061, found_400, found_401;
bool found_402, found_4021, found_4022, found_4031, found_4032, found_4043;
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    */
    case MSG_TXLOCK_REQUEST:
        return fastsend.AlreadyHave(inv.hash);
    ...
    case MSG_DSTX: {
        return static_cast<bool>(CAnonSend::GetDSTX(inv.hash));
    }
        ...
        LogPrint("net", "ProcessGetData -- inv = %s\n", inv.ToString());
        {
            ...
            else if (inv.IsKnownType())
            {
                ...
                if (!push && inv.type == MSG_TXLOCK_REQUEST) {
                    ...
                    if(fastsend.GetTxLockRequest(inv.hash, txLockRequest)) {
                ...
                if (!push && inv.type == MSG_TXLOCK_VOTE) {
                    CTxLockVote vote;
                    if(fastsend.GetTxLockVote(inv.hash, vote)) {
                ...
                if (!push && inv.type == MSG_DSTX) {
                    CAnonsendBroadcastTx dstx = CAnonSend::GetDSTX(inv.hash);
                    if(dstx) {
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman& connman, const std::atomic<bool>& interruptMsgProc)
{
	...
	found_4031 = boost::contains(cleanSubVer, "/MarteX Core:4.0.3.1/");
	found_4032 = boost::contains(cleanSubVer, "/MarteX Core:4.0.3.2/");
	found_4043 = boost::contains(cleanSubVer, "/MarteX Core:4.0.4.3/");

        if (found_300 || found_3001 || found_3021 || found_3022 || found_3031 || found_3032 || found_3033 || found_304
		 || found_3041 || found_3042 || found_3051 || found_3061 || found_400 || found_401 || found_402
		 || found_4021 || found_4022 || found_4031 || found_4032 || found_4043)
        {
            version_old = "< 4.0.4.1";
    ...
    else if (strCommand == NetMsgType::TX || strCommand == NetMsgType::DSTX || strCommand == NetMsgType::TXLOCKREQUEST)
    {
        ...
        if (strCommand == NetMsgType::TXLOCKREQUEST) {
            if(!fastsend.ProcessTxLockRequest(txLockRequest, connman)) {
                LogPrint("fastsend", "TXLOCKREQUEST -- failed %s\n", txLockRequest.GetHash().ToString());
                return false;
            }
        } else if (strCommand == NetMsgType::DSTX) {
            uint256 hashTx = tx.GetHash();
            if(CAnonSend::GetDSTX(hashTx)) {
                LogPrint("anonsend", "DSTX -- Already have %s, skipping...\n", hashTx.ToString());
            ...
            if(!mnodeman.Get(dstx.masternodeOutpoint, mn)) {
                LogPrint("anonsend", "DSTX -- Can't find masternode %s to verify %s\n", dstx.masternodeOutpoint.ToStringShort(), hashTx.ToString());
                return false;
            }

            if(!mn.fAllowMixingTx) {
                LogPrint("anonsend", "DSTX -- Masternode %s is sending too many transactions %s\n", dstx.masternodeOutpoint.ToStringShort(), hashTx.ToString());
                return true;
            ...
            if(!dstx.CheckSignature(mn.pubKeyMasternode)) {
                LogPrint("anonsend", "DSTX -- CheckSignature() failed for %s\n", hashTx.ToString());
                return false;
            }
        ...
        if (!AlreadyHave(inv) && AcceptToMemoryPool(mempool, state, ptx, true, &fMissingInputs, &lRemovedTxn)) {
            ...
            if (strCommand == NetMsgType::DSTX) {
                ...
                CAnonSend::AddDSTX(dstx);
            } else if (strCommand == NetMsgType::TXLOCKREQUEST) {
                LogPrintf("TXLOCKREQUEST -- Transaction Lock Request accepted, txid=%s, peer=%d\n",
                        tx.GetHash().ToString(), pfrom->id);
                fastsend.AcceptLockRequest(txLockRequest);
                fastsend.Vote(tx.GetHash(), connman);
            }
        ...
        else if (fMissingInputs)
        {
            ...
            if (strCommand == NetMsgType::TXLOCKREQUEST && !AlreadyHave(inv)) {
                ...
                fastsend.RejectLockRequest(txLockRequest);
...
#ifdef ENABLE_WALLET
            anonSendClient.ProcessMessage(pfrom, strCommand, vRecv, connman);
#endif // ENABLE_WALLET
            anonSendServer.ProcessMessage(pfrom, strCommand, vRecv, connman);
            mnodeman.ProcessMessage(pfrom, strCommand, vRecv, connman);
            mnpayments.ProcessMessage(pfrom, strCommand, vRecv, connman);
            fastsend.ProcessMessage(pfrom, strCommand, vRecv, connman);
