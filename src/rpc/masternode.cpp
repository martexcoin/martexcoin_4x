...
UniValue anonsend(const JSONRPCRequest& request)
{
    ...
    if(request.params[0].get_str() == "start") {
        ...
        anonSendClient.fEnableAnonSend = true;
        bool result = anonSendClient.DoAutomaticDenominating(*g_connman);
        return "Mixing " + (result ? "started successfully" : ("start failed: " + anonSendClient.GetStatus() + ", will retry"));
    }
    ...
    return "Unknown command, please see \"help anonsend\"";
}
#endif // ENABLE_WALLET
...
UniValue getpoolinfo(const JSONRPCRequest& request)
{
...
#ifdef ENABLE_WALLET
    CAnonSendBase* panonSendBase = fMasternodeMode ? (CAnonSendBase*)&anonSendServer : (CAnonSendBase*)&anonSendClient;

    UniValue obj(UniValue::VOBJ);
    obj.push_back(Pair("state",             panonSendBase->GetStateString()));
    obj.push_back(Pair("mixing_mode",       (!fMasternodeMode && anonSendClient.fAnonSendMultiSession) ? "multi-session" : "normal"));
    obj.push_back(Pair("queue",             panonSendBase->GetQueueSize()));
    obj.push_back(Pair("entries",           panonSendBase->GetEntriesCount()));
    obj.push_back(Pair("status",            anonSendClient.GetStatus()));

    masternode_info_t mnInfo;
    if (anonSendClient.GetMixingMasternodeInfo(mnInfo)) {
    ...
    if (pwalletMain) {
        ...
        obj.push_back(Pair("warnings",      pwalletMain->nKeysLeftSinceAutoBackup < ANONSEND_KEYS_THRESHOLD_WARNING
                                                ? "WARNING: keypool is almost depleted!" : ""));
    }
#else // ENABLE_WALLET
    UniValue obj(UniValue::VOBJ);
    obj.push_back(Pair("state",             anonSendServer.GetStateString()));
    obj.push_back(Pair("queue",             anonSendServer.GetQueueSize()));
    obj.push_back(Pair("entries",           anonSendServer.GetEntriesCount()));
#endif // ENABLE_WALLET
...
UniValue masternode(const JSONRPCRequest& request)
{
    ...
    if (strCommand == "count")
    {
        ...
        int ps = mnodeman.CountEnabled(MIN_ANONSEND_PEER_PROTO_VERSION);
...
UniValue masternodelist(const JSONRPCRequest& request)
{
                ...
                strMode != "rank" && strMode != "sentinel" && strMode != "status"))
    {
        throw std::runtime_error(
                ...
                "  payee          - Print MarteX address associated with a masternode (can be additionally filtered,\n"
                "                   partial match)\n"
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    { "MarteX",               "masternode",             &masternode,             true,  {} },
    { "MarteX",               "masternodelist",         &masternodelist,         true,  {} },
    { "MarteX",               "masternodebroadcast",    &masternodebroadcast,    true,  {} },
    { "MarteX",               "getpoolinfo",            &getpoolinfo,            true,  {} },
    { "MarteX",               "sentinelping",           &sentinelping,           true,  {} },
#ifdef ENABLE_WALLET
    { "MarteX",               "anonsend",            &anonsend,            false, {} },
#endif // ENABLE_WALLET
};
