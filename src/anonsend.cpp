...
bool CAnonSendEntry::AddScriptSig(const CTxIn& txin)
{
...
uint256 CAnonsendQueue::GetSignatureHash() const
{
...
bool CAnonsendQueue::Sign()
{
    ...
    if (sporkManager.IsSporkActive(SPORK_6_NEW_SIGS)) {
        ...
        if (!CHashSigner::SignHash(hash, activeMasternode.keyMasternode, vchSig)) {
            LogPrintf("CAnonsendQueue::Sign -- SignHash() failed\n");
            return false;
        }

        if (!CHashSigner::VerifyHash(hash, activeMasternode.pubKeyMasternode, vchSig, strError)) {
            LogPrintf("CAnonsendQueue::Sign -- VerifyHash() failed, error: %s\n", strError);
            return false;
        }
    } else {
        ...
        if(!CMessageSigner::SignMessage(strMessage, vchSig, activeMasternode.keyMasternode)) {
            LogPrintf("CAnonsendQueue::Sign -- SignMessage() failed, %s\n", ToString());
            return false;
        }

        if(!CMessageSigner::VerifyMessage(activeMasternode.pubKeyMasternode, vchSig, strMessage, strError)) {
            LogPrintf("CAnonsendQueue::Sign -- VerifyMessage() failed, error: %s\n", strError);
            return false;
        }
    }
...
bool CAnonsendQueue::CheckSignature(const CPubKey& pubKeyMasternode) const
{
    std::string strError = "";
    ...
    if (sporkManager.IsSporkActive(SPORK_6_NEW_SIGS)) {
        ...
        if (!CHashSigner::VerifyHash(hash, pubKeyMasternode, vchSig, strError)) {
            ...
            LogPrintf("CAnonsendQueue::CheckSignature -- VerifyHash() failed, error: %s\n", strError);
            return false;
        }
    } else {
        std::string strMessage = CTxIn(masternodeOutpoint).ToString() +
        ...
        if(!CMessageSigner::VerifyMessage(pubKeyMasternode, vchSig, strMessage, strError)) {
            LogPrintf("CAnonsendQueue::CheckSignature -- Got bad Masternode queue signature: %s; error: %s\n", ToString(), strError);
            return false;
        }
    }
...
bool CAnonsendQueue::Relay(CConnman& connman)
{
    connman.ForEachNode([&connman, this](CNode* pnode) {
        CNetMsgMaker msgMaker(pnode->GetSendVersion());
        if (pnode->nVersion >= MIN_ANONSEND_PEER_PROTO_VERSION)
            connman.PushMessage(pnode, msgMaker.Make(NetMsgType::DSQUEUE, (*this)));
    });
    return true;
}

uint256 CAnonsendBroadcastTx::GetSignatureHash() const
{
    return SerializeHash(*this);
}

bool CAnonsendBroadcastTx::Sign()
{
    if(!fMasternodeMode) return false;
    ...
    if (sporkManager.IsSporkActive(SPORK_6_NEW_SIGS)) {
        ...
        if (!CHashSigner::SignHash(hash, activeMasternode.keyMasternode, vchSig)) {
            LogPrintf("CAnonsendBroadcastTx::Sign -- SignHash() failed\n");
            return false;
        }

        if (!CHashSigner::VerifyHash(hash, activeMasternode.pubKeyMasternode, vchSig, strError)) {
            LogPrintf("CAnonsendBroadcastTx::Sign -- VerifyHash() failed, error: %s\n", strError);
            return false;
        }
    } else {
        ...
        if(!CMessageSigner::SignMessage(strMessage, vchSig, activeMasternode.keyMasternode)) {
            LogPrintf("CAnonsendBroadcastTx::Sign -- SignMessage() failed\n");
            return false;
        }

        if(!CMessageSigner::VerifyMessage(activeMasternode.pubKeyMasternode, vchSig, strMessage, strError)) {
            LogPrintf("CAnonsendBroadcastTx::Sign -- VerifyMessage() failed, error: %s\n", strError);
            return false;
        }
    }
...
bool CAnonsendBroadcastTx::CheckSignature(const CPubKey& pubKeyMasternode) const
{
    std::string strError = "";
    ...
    if (sporkManager.IsSporkActive(SPORK_6_NEW_SIGS)) {
        ...
        if (!CHashSigner::VerifyHash(hash, pubKeyMasternode, vchSig, strError)) {
            ...
            LogPrintf("CAnonsendBroadcastTx::CheckSignature -- VerifyHash() failed, error: %s\n", strError);
            return false;
        }
    } else {
        std::string strMessage = tx->GetHash().ToString() + boost::lexical_cast<std::string>(sigTime);
        ...
        if(!CMessageSigner::VerifyMessage(pubKeyMasternode, vchSig, strMessage, strError)) {
            LogPrintf("CAnonsendBroadcastTx::CheckSignature -- Got bad dstx signature, error: %s\n", strError);
            return false;
        }
    }
...
void CAnonSendBase::CheckQueue()
{
    ...
    while(it != vecAnonsendQueue.end()) {
        if((*it).IsExpired()) {
            LogPrint("anonsend", "CAnonSendBase::%s -- Removing expired queue (%s)\n", __func__, (*it).ToString());
            it = vecAnonsendQueue.erase(it);
        } else ++it;
    }
}

std::string CAnonSendBase::GetStateString() const
{
    switch(nState) {
...
std::vector<CAmount> CAnonSend::vecStandardDenominations;
std::map<uint256, CAnonsendBroadcastTx> CAnonSend::mapDSTX;
CCriticalSection CAnonSend::cs_mapdstx;

void CAnonSend::InitStandardDenominations()
{
...
bool CAnonSend::IsCollateralValid(const CTransaction& txCollateral)
{
    ...
    for (const auto& txout : txCollateral.vout) {
        ...
        if(!txout.scriptPubKey.IsPayToPublicKeyHash() && !(fAllowData && txout.scriptPubKey.IsUnspendable())) {
            LogPrintf ("CAnonSend::IsCollateralValid -- Invalid Script, txCollateral=%s", txCollateral.ToString());
            return false;
        }
    }
    ...
    for (const auto& txin : txCollateral.vin) {
        ...
        if(!GetUTXOCoin(txin.prevout, coin)) {
            LogPrint("anonsend", "CAnonSend::IsCollateralValid -- Unknown inputs in collateral transaction, txCollateral=%s", txCollateral.ToString());
            return false;
        }
    ...
    if(nValueIn - nValueOut < GetCollateralAmount()) {
        LogPrint("anonsend", "CAnonSend::IsCollateralValid -- did not include enough fees in transaction: fees: %d, txCollateral=%s", nValueOut - nValueIn, txCollateral.ToString());
        return false;
    }

    LogPrint("anonsend", "CAnonSend::IsCollateralValid -- %s", txCollateral.ToString());

    {
        LOCK(cs_main);
        CValidationState validationState;
        if(!AcceptToMemoryPool(mempool, validationState, MakeTransactionRef(txCollateral), false, NULL, NULL, false, maxTxFee, true)) {
            LogPrint("anonsend", "CAnonSend::IsCollateralValid -- didn't pass AcceptToMemoryPool()\n");
            return false;
        }
    }
...
bool CAnonSend::IsCollateralAmount(CAmount nInputAmount)
{
...
*/
std::string CAnonSend::GetDenominationsToString(int nDenom)
{
    std::string strDenom = "";
...
*/
int CAnonSend::GetDenominations(const std::vector<CTxOut>& vecTxOut, bool fSingleRandomDenom)
{
...
bool CAnonSend::GetDenominationsBits(int nDenom, std::vector<int> &vecBitsRet)
{
...
int CAnonSend::GetDenominationsByAmounts(const std::vector<CAmount>& vecAmount)
{
...
bool CAnonSend::IsDenominatedAmount(CAmount nInputAmount)
{
    for (const auto& nDenomValue : vecStandardDenominations)
...
std::string CAnonSend::GetMessageByID(PoolMessage nMessageID)
{
    switch (nMessageID) {
...
void CAnonSend::AddDSTX(const CAnonsendBroadcastTx& dstx)
{
    LOCK(cs_mapdstx);
    mapDSTX.insert(std::make_pair(dstx.tx->GetHash(), dstx));
}

CAnonsendBroadcastTx CAnonSend::GetDSTX(const uint256& hash)
{
    LOCK(cs_mapdstx);
    auto it = mapDSTX.find(hash);
    return (it == mapDSTX.end()) ? CAnonsendBroadcastTx() : it->second;
}

void CAnonSend::CheckDSTXes(int nHeight)
{
    LOCK(cs_mapdstx);
    std::map<uint256, CAnonsendBroadcastTx>::iterator it = mapDSTX.begin();
    ...
    LogPrint("anonsend", "CAnonSend::CheckDSTXes -- mapDSTX.size()=%llu\n", mapDSTX.size());
}

void CAnonSend::UpdatedBlockTip(const CBlockIndex *pindex)
{
...
void CAnonSend::SyncTransaction(const CTransaction& tx, const CBlockIndex *pindex, int posInBlock)
{
    ...
    LogPrint("anonsend", "CAnonSendClient::SyncTransaction -- txid=%s\n", txHash.ToString());
}

//TODO: Rename/move to core
void ThreadCheckAnonSend(CConnman& connman)
{
