...
extern CFastSend fastsend;

/*
    At 15 signatures, 1/2 of the masternode network can be owned by
    one party without compromising the security of FastSend
...
// The FASTSEND_DEPTH is the "pseudo block depth" level assigned to locked
...
static const int FASTSEND_LOCK_TIMEOUT_SECONDS   = 15;
/// For how long we are going to keep invalid votes and votes for failed lock attempts,
/// must be greater than FASTSEND_LOCK_TIMEOUT_SECONDS
static const int FASTSEND_FAILED_TIMEOUT_SECONDS = 60;
...
/**
 * Manages FastSend. Processes lock requests, candidates, and votes.
 */
class CFastSend
{
private:
    ...
    bool IsFastSendReadyToLock(const uint256 &txHash);

public:
    CCriticalSection cs_fastsend;
...
/**
 * An FastSend transaction lock request.
 */
class CTxLockRequest
{
private:
...
/**
 * An FastSend transaction lock vote. Sent by a masternode in response to a
...
/**
 * An FastSend transaction lock candidate.
 */
class CTxLockCandidate
{
private:
