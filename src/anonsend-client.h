...
#ifndef ANONSENDCLIENT_H
#define ANONSENDCLIENT_H

#include "masternode.h"
#include "anonsend.h"
#include "wallet/wallet.h"
#include "anonsend-util.h"

class CAnonSendClient;
class CConnman;

static const int DENOMS_COUNT_MAX                   = 100;

static const int MIN_ANONSEND_ROUNDS             = 2;
static const int MIN_ANONSEND_AMOUNT             = 2;
static const int MIN_ANONSEND_LIQUIDITY          = 0;
static const int MAX_ANONSEND_ROUNDS             = 16;
static const int MAX_ANONSEND_AMOUNT             = MAX_MONEY / COIN;
static const int MAX_ANONSEND_LIQUIDITY          = 100;
static const int DEFAULT_ANONSEND_ROUNDS         = 2;
static const int DEFAULT_ANONSEND_AMOUNT         = 1000;
static const int DEFAULT_ANONSEND_LIQUIDITY      = 0;

static const bool DEFAULT_ANONSEND_MULTISESSION  = false;

// Warn user if mixing in gui or try to create backup if mixing in daemon mode
// when we have only this many keys left
static const int ANONSEND_KEYS_THRESHOLD_WARNING = 100;
// Stop mixing completely, it's too dangerous to continue when we have only this many keys left
static const int ANONSEND_KEYS_THRESHOLD_STOP    = 50;

// The main object for accessing mixing
extern CAnonSendClient anonSendClient;
...
public:
    ...
    CPendingDsaRequest(const CService& addr_, const CAnonsendAccept& dsa_):
        addr(addr_),
        dsa(dsa_)
    { nTimeCreated = GetTime(); }

    CService GetAddr() { return addr; }
    CAnonsendAccept GetDSA() { return dsa; }
 ...
 */
class CAnonSendClient : public CAnonSendBase
{
private:
    ...
    void RelayIn(const CAnonSendEntry& entry, CConnman& connman);
...
public:
    ...
    bool fEnableAnonSend;
    bool fAnonSendMultiSession;
    ...
    CAnonSendClient() :
        ...
        nAnonSendRounds(DEFAULT_ANONSEND_ROUNDS),
        nAnonSendAmount(DEFAULT_ANONSEND_AMOUNT),
        nLiquidityProvider(DEFAULT_ANONSEND_LIQUIDITY),
        fEnableAnonSend(false),
        fAnonSendMultiSession(DEFAULT_ANONSEND_MULTISESSION),
