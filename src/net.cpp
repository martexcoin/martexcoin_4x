...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "MarteX Core " + FormatFullVersion();
...
void CConnman::RelayTransaction(const CTransaction& tx)
{
    ...
    int nInv = static_cast<bool>(CAnonSend::GetDSTX(hash)) ? MSG_DSTX :
                (fastsend.HasTxLockRequest(hash) ? MSG_TXLOCK_REQUEST : MSG_TX);
    CInv inv(nInv, hash);
    LOCK(cs_vNodes);
    BOOST_FOREACH(CNode* pnode, vNodes)
    {
