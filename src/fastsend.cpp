...
int nFastSendDepth = DEFAULT_FASTSEND_DEPTH;
...
void CFastSend::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (strCommand == NetMsgType::TXLOCKVOTE) // FastSend Transaction Lock Consensus Votes
    {
        if(pfrom->nVersion < MIN_FASTSEND_PROTO_VERSION) {
            LogPrint("fastsend", "TXLOCKVOTE -- peer=%d using obsolete version %i\n", pfrom->id, pfrom->nVersion);
            connman.PushMessage(pfrom, CNetMsgMaker(pfrom->GetSendVersion()).Make(NetMsgType::REJECT, strCommand, REJECT_OBSOLETE,
                               strprintf("Version must be %d or greater", MIN_FASTSEND_PROTO_VERSION)));
            return;
        }
        ...
        // Ignore any FastSend messages until masternode list is synced
...
bool CFastSend::ProcessTxLockRequest(const CTxLockRequest& txLockRequest, CConnman& connman)
{
    LOCK(cs_main);
#ifdef ENABLE_WALLET
    LOCK(pwalletMain ? &pwalletMain->cs_wallet : NULL);
#endif
    LOCK2(mempool.cs, cs_fastsend);
    ...
    for (const auto& txin : txLockRequest.tx->vin) {
        ...
        if(it != mapLockedOutpoints.end() && it->second != txLockRequest.GetHash()) {
            ...
            LogPrintf("CFastSend::ProcessTxLockRequest -- WARNING: Found conflicting completed Transaction Lock, txid=%s, completed lock txid=%s\n",
        ...
        if(it != mapVotedOutpoints.end()) {
            for (const auto& hash : it->second) {
                if(hash != txLockRequest.GetHash()) {
                    LogPrint("fastsend", "CFastSend::ProcessTxLockRequest -- Double spend attempt! %s\n", txin.prevout.ToStringShort());
    ...
    if(!CreateTxLockCandidate(txLockRequest)) {
        // smth is not right
        LogPrintf("CFastSend::ProcessTxLockRequest -- CreateTxLockCandidate failed, txid=%s\n", txHash.ToString());
        return false;
    }
    LogPrintf("CFastSend::ProcessTxLockRequest -- accepted, txid=%s\n", txHash.ToString());
...
bool CFastSend::CreateTxLockCandidate(const CTxLockRequest& txLockRequest)
{
    ...
    if(itLockCandidate == mapTxLockCandidates.end()) {
        LogPrintf("CFastSend::CreateTxLockCandidate -- new, txid=%s\n", txHash.ToString());
        ...
        if (itLockCandidate->second.IsTimedOut()) {
            LogPrintf("CFastSend::CreateTxLockCandidate -- timed out, txid=%s\n", txHash.ToString());
            return false;
        }
        LogPrintf("CFastSend::CreateTxLockCandidate -- update empty, txid=%s\n", txHash.ToString());
        ...
        LogPrint("fastsend", "CFastSend::CreateTxLockCandidate -- seen, txid=%s\n", txHash.ToString());
    }

    return true;
}

void CFastSend::CreateEmptyTxLockCandidate(const uint256& txHash)
{
    if (mapTxLockCandidates.find(txHash) != mapTxLockCandidates.end())
        return;
    LogPrintf("CFastSend::CreateEmptyTxLockCandidate -- new, txid=%s\n", txHash.ToString());
...
void CFastSend::Vote(const uint256& txHash, CConnman& connman)
{
...
void CFastSend::Vote(CTxLockCandidate& txLockCandidate, CConnman& connman)
{
    if(!fMasternodeMode) return;
    if(!sporkManager.IsSporkActive(SPORK_2_FASTSEND_ENABLED)) return;

    AssertLockHeld(cs_main);
    AssertLockHeld(cs_fastsend);
    ...
    while(itOutpointLock != txLockCandidate.mapOutPointLocks.end()) {
        ...
        if(nPrevoutHeight == -1) {
            LogPrint("fastsend", "CFastSend::Vote -- Failed to find UTXO %s\n", itOutpointLock->first.ToStringShort());
            return;
        }

        int nLockInputHeight = nPrevoutHeight + Params().GetConsensus().nFastSendConfirmationsRequired - 2;

        int nRank;
        int nMinRequiredProtocol = std::max(MIN_FASTSEND_PROTO_VERSION, mnpayments.GetMinMasternodePaymentsProto());
        if(!mnodeman.GetMasternodeRank(activeMasternode.outpoint, nRank, nLockInputHeight, nMinRequiredProtocol)) {
            LogPrint("fastsend", "CFastSend::Vote -- Can't calculate rank for masternode %s\n", activeMasternode.outpoint.ToStringShort());
            ++itOutpointLock;
            continue;
        }
        ...
        if(nRank > nSignaturesTotal) {
            LogPrint("fastsend", "CFastSend::Vote -- Masternode not in the top %d (%d)\n", nSignaturesTotal, nRank);
            ++itOutpointLock;
            continue;
        }

        LogPrint("fastsend", "CFastSend::Vote -- In the top %d (%d)\n", nSignaturesTotal, nRank);
        ...
        if(itVoted != mapVotedOutpoints.end()) {
            for (const auto& hash : itVoted->second) {
                ...
                if(it2->second.HasMasternodeVoted(itOutpointLock->first, activeMasternode.outpoint)) {
                    ...
                    // skip it anyway
                    fAlreadyVoted = true;
                    LogPrintf("CFastSend::Vote -- WARNING: We already voted for this outpoint, skipping: txHash=%s, outpoint=%s\n",
        ...
        if(!vote.Sign()) {
            LogPrintf("CFastSend::Vote -- Failed to sign consensus vote\n");
            return;
        }
        if(!vote.CheckSignature()) {
            LogPrintf("CFastSend::Vote -- Signature invalid\n");
            return;
        }
        ...
        if(itOutpointLock->second.AddVote(vote)) {
            LogPrintf("CFastSend::Vote -- Vote created successfully, relaying: txHash=%s, outpoint=%s, vote=%s\n",
            ...
            if(itVoted == mapVotedOutpoints.end()) {
                ...
                if(mapVotedOutpoints[itOutpointLock->first].size() > 1) {
                    ...
                    LogPrintf("CFastSend::Vote -- WARNING: Vote conflicts with some existing votes: txHash=%s, outpoint=%s, vote=%s\n",
                            txHash.ToString(), itOutpointLock->first.ToStringShort(), nVoteHash.ToString());
                }
            }
...
bool CFastSend::ProcessNewTxLockVote(CNode* pfrom, const CTxLockVote& vote, CConnman& connman)
{
    ...
    if(!vote.IsValid(pfrom, connman)) {
        ...
        LogPrint("fastsend", "CFastSend::%s -- Vote is invalid, txid=%s\n", __func__, txHash.ToString());
        return false;
    }
    ...
    if(it == mapTxLockCandidates.end() || !it->second.txLockRequest) {
        ...
        LogPrint("fastsend", "CFastSend::%s -- Orphan vote: txid=%s  masternode=%s %s\n",
        ...
        if(itMnOV == mapMasternodeOrphanVotes.end()) {
            ...
            if(itMnOV->second > GetTime() && itMnOV->second > GetAverageMasternodeOrphanVoteTime()) {
                LogPrint("fastsend", "CFastSend::%s -- masternode is spamming orphan Transaction Lock Votes: txid=%s  masternode=%s\n",
    ...
    if (txLockCandidate.IsTimedOut()) {
        LogPrint("fastsend", "CFastSend::%s -- too late, Transaction Lock timed out, txid=%s\n", __func__, txHash.ToString());
        return false;
    }

    LogPrint("fastsend", "CFastSend::%s -- Transaction Lock Vote, txid=%s\n", __func__, txHash.ToString());
    ...
    LogPrint("fastsend", "CFastSend::%s -- Transaction Lock signatures count: %d/%d, vote hash=%s\n", __func__,
...
bool CFastSend::ProcessOrphanTxLockVote(const CTxLockVote& vote)
{
    // cs_main, cs_wallet and cs_fastsend should be already locked
    AssertLockHeld(cs_main);
#ifdef ENABLE_WALLET
    if (pwalletMain)
...
void CFastSend::UpdateVotedOutpoints(const CTxLockVote& vote, CTxLockCandidate& txLockCandidate)
{
    AssertLockHeld(cs_fastsend);
        ...
        for (const auto& hash : it1->second) {
            if(hash != txHash) {
                ...
                if(it2 !=mapTxLockCandidates.end() && it2->second.HasMasternodeVoted(vote.GetOutpoint(), vote.GetMasternodeOutpoint())) {
                    ...
                    LogPrintf("CFastSend::%s -- masternode sent conflicting votes! %s\n", __func__, vote.GetMasternodeOutpoint().ToStringShort());
...
void CFastSend::ProcessOrphanTxLockVotes()
{
    AssertLockHeld(cs_main);
    AssertLockHeld(cs_fastsend);
...
void CFastSend::TryToFinalizeLockCandidate(const CTxLockCandidate& txLockCandidate)
{
    if(!sporkManager.IsSporkActive(SPORK_2_FASTSEND_ENABLED)) return;
    ...
    if(txLockCandidate.IsAllOutPointsReady() && !IsLockedFastSendTransaction(txHash)) {
        // we have enough votes now
        LogPrint("fastsend", "CFastSend::TryToFinalizeLockCandidate -- Transaction Lock is ready to complete, txid=%s\n", txHash.ToString());
...
void CFastSend::UpdateLockedTransaction(const CTxLockCandidate& txLockCandidate)
{
    // cs_main, cs_wallet and cs_fastsend should be already locked
    AssertLockHeld(cs_main);
#ifdef ENABLE_WALLET
    ...
    if(!IsLockedFastSendTransaction(txHash)) return; // not a locked tx, do not update/notify
    ...
    if(pwalletMain && pwalletMain->UpdatedTransaction(txHash)) {
        ...
        std::string strCmd = GetArg("-fastsendnotify", "");
        if(!strCmd.empty()) {
    ...
    LogPrint("fastsend", "CFastSend::UpdateLockedTransaction -- done, txid=%s\n", txHash.ToString());
}

void CFastSend::LockTransactionInputs(const CTxLockCandidate& txLockCandidate)
{
    if(!sporkManager.IsSporkActive(SPORK_2_FASTSEND_ENABLED)) return;
    ...
    LogPrint("fastsend", "CFastSend::LockTransactionInputs -- done, txid=%s\n", txHash.ToString());
}

bool CFastSend::GetLockedOutPointTxHash(const COutPoint& outpoint, uint256& hashRet)
{
    LOCK(cs_fastsend);
...
bool CFastSend::ResolveConflicts(const CTxLockCandidate& txLockCandidate)
{
    AssertLockHeld(cs_main);
    AssertLockHeld(cs_fastsend);
    ...
    for (const auto& txin : txLockCandidate.txLockRequest.tx->vin) {
        ...
        if(GetLockedOutPointTxHash(txin.prevout, hashConflicting) && txHash != hashConflicting) {
            ...
            if(itLockCandidate == mapTxLockCandidates.end() || itLockCandidateConflicting == mapTxLockCandidates.end()) {
                ...
                LogPrintf("CFastSend::ResolveConflicts -- ERROR: Found conflicting completed Transaction Lock, but one of txLockCandidate-s is missing, txid=%s, conflicting txid=%s\n",
                        txHash.ToString(), hashConflicting.ToString());
                return false;
            }
            LogPrintf("CFastSend::ResolveConflicts -- WARNING: Found conflicting completed Transaction Lock, dropping both, txid=%s, conflicting txid=%s\n",
                    txHash.ToString(), hashConflicting.ToString());
            ...
            LogPrintf("CFastSend::ResolveConflicts -- ERROR: Failed to complete Transaction Lock, conflicts with mempool, txid=%s\n", txHash.ToString());
            return false;
        }
    } // FOREACH
    ...
    if(GetTransaction(txHash, txTmp, Params().GetConsensus(), hashBlock, true) && hashBlock != uint256()) {
        LogPrint("fastsend", "CFastSend::ResolveConflicts -- Done, %s is included in block %s\n", txHash.ToString(), hashBlock.ToString());
        return true;
    }
    ...
    for (const auto& txin : txLockCandidate.txLockRequest.tx->vin) {
        ...
        if(!GetUTXOCoin(txin.prevout, coin)) {
            ...
            LogPrintf("CFastSend::ResolveConflicts -- ERROR: Failed to find UTXO %s, can't complete Transaction Lock\n", txin.prevout.ToStringShort());
            return false;
        }
    }
    LogPrint("fastsend", "CFastSend::ResolveConflicts -- Done, txid=%s\n", txHash.ToString());

    return true;
}

int64_t CFastSend::GetAverageMasternodeOrphanVoteTime()
{
    LOCK(cs_fastsend);
...
void CFastSend::CheckAndRemove()
{
    ...
    while(itLockCandidate != mapTxLockCandidates.end()) {
        ...
        if(txLockCandidate.IsExpired(nCachedBlockHeight)) {
            LogPrintf("CFastSend::CheckAndRemove -- Removing expired Transaction Lock Candidate: txid=%s\n", txHash.ToString());
            std::map<COutPoint, COutPointLock>::iterator itOutpointLock = txLockCandidate.mapOutPointLocks.begin();
            while(itOutpointLock != txLockCandidate.mapOutPointLocks.end()) {
    ...
    while(itVote != mapTxLockVotes.end()) {
        if(itVote->second.IsExpired(nCachedBlockHeight)) {
            LogPrint("fastsend", "CFastSend::CheckAndRemove -- Removing expired vote: txid=%s  masternode=%s\n",
    ...
    while(itOrphanVote != mapTxLockVotesOrphan.end()) {
        if(itOrphanVote->second.IsTimedOut()) {
            LogPrint("fastsend", "CFastSend::CheckAndRemove -- Removing timed out orphan vote: txid=%s  masternode=%s\n",
    ...
    while(itVote != mapTxLockVotes.end()) {
        if(itVote->second.IsFailed()) {
            LogPrint("fastsend", "CFastSend::CheckAndRemove -- Removing vote for failed lock attempt: txid=%s  masternode=%s\n",
                    itVote->second.GetTxHash().ToString(), itVote->second.GetMasternodeOutpoint().ToStringShort());
            mapTxLockVotes.erase(itVote++);
        } else {
            ++itVote;
    ...
    while(itMasternodeOrphan != mapMasternodeOrphanVotes.end()) {
        if(itMasternodeOrphan->second < GetTime()) {
            LogPrint("fastsend", "CFastSend::CheckAndRemove -- Removing timed out orphan masternode vote: masternode=%s\n",
    ...
    LogPrintf("CFastSend::CheckAndRemove -- %s\n", ToString());
}

bool CFastSend::AlreadyHave(const uint256& hash)
{
    LOCK(cs_fastsend);
...
void CFastSend::AcceptLockRequest(const CTxLockRequest& txLockRequest)
{
    LOCK(cs_fastsend);
    mapLockRequestAccepted.insert(std::make_pair(txLockRequest.GetHash(), txLockRequest));
}

void CFastSend::RejectLockRequest(const CTxLockRequest& txLockRequest)
{
    LOCK(cs_fastsend);
    mapLockRequestRejected.insert(std::make_pair(txLockRequest.GetHash(), txLockRequest));
}

bool CFastSend::HasTxLockRequest(const uint256& txHash)
{
    CTxLockRequest txLockRequestTmp;
    return GetTxLockRequest(txHash, txLockRequestTmp);
}

bool CFastSend::GetTxLockRequest(const uint256& txHash, CTxLockRequest& txLockRequestRet)
{
    LOCK(cs_fastsend);
...
bool CFastSend::GetTxLockVote(const uint256& hash, CTxLockVote& txLockVoteRet)
{
    LOCK(cs_fastsend);
...
bool CFastSend::IsFastSendReadyToLock(const uint256& txHash)
{
    if(!fEnableFastSend || GetfLargeWorkForkFound() || GetfLargeWorkInvalidChainFound() ||
        !sporkManager.IsSporkActive(SPORK_2_FASTSEND_ENABLED)) return false;
...
bool CFastSend::IsLockedFastSendTransaction(const uint256& txHash)
{
    if(!fEnableFastSend || GetfLargeWorkForkFound() || GetfLargeWorkInvalidChainFound() ||
        !sporkManager.IsSporkActive(SPORK_3_FASTSEND_BLOCK_FILTERING)) return false;
...
int CFastSend::GetTransactionLockSignatures(const uint256& txHash)
{
...
int CFastSend::GetConfirmations(const uint256 &nTXHash)
{
    return IsLockedFastSendTransaction(nTXHash) ? nFastSendDepth : 0;
}

bool CFastSend::IsTxLockCandidateTimedOut(const uint256& txHash)
{
    if(!fEnableFastSend) return false;
...
void CFastSend::Relay(const uint256& txHash, CConnman& connman)
{
    LOCK(cs_fastsend);
...
void CFastSend::UpdatedBlockTip(const CBlockIndex *pindex)
{
    nCachedBlockHeight = pindex->nHeight;
}

void CFastSend::SyncTransaction(const CTransaction& tx, const CBlockIndex *pindex, int posInBlock)
{
    ...
    LogPrint("fastsend", "CFastSend::SyncTransaction -- txid=%s nHeightNew=%d\n", txHash.ToString(), nHeightNew);

    // Check lock candidates
    std::map<uint256, CTxLockCandidate>::iterator itLockCandidate = mapTxLockCandidates.find(txHash);
    if(itLockCandidate != mapTxLockCandidates.end()) {
        LogPrint("fastsend", "CFastSend::SyncTransaction -- txid=%s nHeightNew=%d lock candidate updated\n",
                txHash.ToString(), nHeightNew);
        ...
        while(itOutpointLock != itLockCandidate->second.mapOutPointLocks.end()) {
            ...
            while(itVote != vVotes.end()) {
                ...
                LogPrint("fastsend", "CFastSend::SyncTransaction -- txid=%s nHeightNew=%d vote %s updated\n",
...
bool CTxLockRequest::IsValid() const
{
    ...
    if(tx->vin.size() > WARN_MANY_INPUTS) {
        LogPrint("fastsend", "CTxLockRequest::IsValid -- WARNING: Too many inputs: tx=%s", ToString());
    }

    AssertLockHeld(cs_main);
    if(!CheckFinalTx(*tx)) {
        LogPrint("fastsend", "CTxLockRequest::IsValid -- Transaction is not final: tx=%s", ToString());
        return false;
    }
    ...
    for (const auto& txin : tx->vin) {
        ...
        if(!GetUTXOCoin(txin.prevout, coin)) {
            LogPrint("fastsend", "CTxLockRequest::IsValid -- Failed to find UTXO %s\n", txin.prevout.ToStringShort());
            return false;
        }
        ...
        int nConfirmationsRequired = nFastSendConfirmationsRequired - 1;

        if(nTxAge < nConfirmationsRequired) {
            LogPrint("fastsend", "CTxLockRequest::IsValid -- outpoint %s too new: nTxAge=%d, nConfirmationsRequired=%d, txid=%s\n",
    ...
    if(nValueIn > sporkManager.GetSporkValue(SPORK_5_FASTSEND_MAX_VALUE)*COIN) {
        LogPrint("fastsend", "CTxLockRequest::IsValid -- Transaction value too high: nValueIn=%d, tx=%s", nValueIn, ToString());
        return false;
    }
    ...
    if(nValueIn - nValueOut < GetMinFee()) {
        LogPrint("fastsend", "CTxLockRequest::IsValid -- did not include enough fees in transaction: fees=%d, tx=%s", nValueOut - nValueIn, ToString());
        return false;
    }
...
bool CTxLockVote::IsValid(CNode* pnode, CConnman& connman) const
{
    if(!mnodeman.Has(outpointMasternode)) {
        LogPrint("fastsend", "CTxLockVote::IsValid -- Unknown masternode %s\n", outpointMasternode.ToStringShort());
    ...
    if(!GetUTXOCoin(outpoint, coin)) {
        LogPrint("fastsend", "CTxLockVote::IsValid -- Failed to find UTXO %s\n", outpoint.ToStringShort());
        return false;
    }

    int nLockInputHeight = coin.nHeight + Params().GetConsensus().nFastSendConfirmationsRequired - 2;

    int nRank;
    int nMinRequiredProtocol = std::max(MIN_FASTSEND_PROTO_VERSION, mnpayments.GetMinMasternodePaymentsProto());
    if(!mnodeman.GetMasternodeRank(outpointMasternode, nRank, nLockInputHeight, nMinRequiredProtocol)) {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("fastsend", "CTxLockVote::IsValid -- Can't calculate rank for masternode %s\n", outpointMasternode.ToStringShort());
        return false;
    }
    LogPrint("fastsend", "CTxLockVote::IsValid -- Masternode %s, rank=%d\n", outpointMasternode.ToStringShort(), nRank);

    int nSignaturesTotal = COutPointLock::SIGNATURES_TOTAL;
    if(nRank > nSignaturesTotal) {
        LogPrint("fastsend", "CTxLockVote::IsValid -- Masternode %s is not in the top %d (%d), vote hash=%s\n",
...
{
    // Locks and votes expire nFastSendKeepLock blocks after the block corresponding tx was included into.
    return (nConfirmedHeight != -1) && (nHeight - nConfirmedHeight > Params().GetConsensus().nFastSendKeepLock);
}

bool CTxLockVote::IsTimedOut() const
{
    return GetTime() - nTimeCreated > FASTSEND_LOCK_TIMEOUT_SECONDS;
}

bool CTxLockVote::IsFailed() const
{
    return (GetTime() - nTimeCreated > FASTSEND_FAILED_TIMEOUT_SECONDS) && !fastsend.IsLockedFastSendTransaction(GetTxHash());
}
