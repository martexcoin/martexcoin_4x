...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    std::string debugCategories = "addrman, alert, bench, cmpctblock, coindb, db, http, leveldb, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                                  "martex (or specifically: gobject, fastsend, keepass, masternode, mnpayments, mnsync, anonsend, spork)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all MarteX specific functionality (Masternodes, AnonSend, FastSend, Governance) (0-1, default: %u)"), 0));
...
#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("AnonSend options:"));
    strUsage += HelpMessageOpt("-enableanonsend=<n>", strprintf(_("Enable use of automated AnonSend for funds stored in this wallet (0-1, default: %u)"), 0));
    strUsage += HelpMessageOpt("-anonsendmultisession=<n>", strprintf(_("Enable multiple AnonSend mixing sessions per block, experimental (0-1, default: %u)"), DEFAULT_ANONSEND_MULTISESSION));
    strUsage += HelpMessageOpt("-anonsendrounds=<n>", strprintf(_("Use N separate masternodes for each denominated input to mix funds (%u-%u, default: %u)"), MIN_ANONSEND_ROUNDS, MAX_ANONSEND_ROUNDS, DEFAULT_ANONSEND_ROUNDS));
    strUsage += HelpMessageOpt("-anonsendamount=<n>", strprintf(_("Keep N MXT anonymized (%u-%u, default: %u)"), MIN_ANONSEND_AMOUNT, MAX_ANONSEND_AMOUNT, DEFAULT_ANONSEND_AMOUNT));
    strUsage += HelpMessageOpt("-liquidityprovider=<n>", strprintf(_("Provide liquidity to AnonSend by infrequently mixing coins on a continual basis (%u-%u, default: %u, 1=very frequent, high fees, %u=very infrequent, low fees)"),
        MIN_ANONSEND_LIQUIDITY, MAX_ANONSEND_LIQUIDITY, DEFAULT_ANONSEND_LIQUIDITY, MAX_ANONSEND_LIQUIDITY));
#endif // ENABLE_WALLET

    strUsage += HelpMessageGroup(_("FastSend options:"));
    strUsage += HelpMessageOpt("-enablefastsend=<n>", strprintf(_("Enable FastSend, show confirmations for locked transactions (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-fastsenddepth=<n>", strprintf(_("Show N confirmations for a successfully locked transaction (%u-%u, default: %u)"), MIN_FASTSEND_DEPTH, MAX_FASTSEND_DEPTH, DEFAULT_FASTSEND_DEPTH));
    strUsage += HelpMessageOpt("-fastsendnotify=<cmd>", _("Execute command when a wallet FastSend transaction is successfully locked (%s in cmd is replaced by TxID)"));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/martexcoin/martexcoin>";
    const std::string URL_WEBSITE = "<https://martexcoin.org>";
...
/** Sanity checks
 *  Ensure that MarteX Core is running in a usable environment with all
...
void InitParameterInteraction()
{
    ...
    if(!GetBoolArg("-enablefastsend", fEnableFastSend)){
        if (SoftSetArg("-fastsenddepth", "0"))
            LogPrintf("%s: parameter interaction: -enablefastsend=false -> setting -nFastSendDepth=0\n", __func__);
    }

#ifdef ENABLE_WALLET
    int nLiqProvTmp = GetArg("-liquidityprovider", DEFAULT_ANONSEND_LIQUIDITY);
    if (nLiqProvTmp > 0) {
        ForceSetArg("-enableanonsend", "1");
        LogPrintf("%s: parameter interaction: -liquidityprovider=%d -> setting -enableanonsend=1\n", __func__, nLiqProvTmp);
        ForceSetArg("-anonsendrounds", itostr(std::numeric_limits<int>::max()));
        LogPrintf("%s: parameter interaction: -liquidityprovider=%d -> setting -anonsendrounds=%d\n", __func__, nLiqProvTmp, itostr(std::numeric_limits<int>::max()));
        ForceSetArg("-anonsendamount", itostr(MAX_ANONSEND_AMOUNT));
        LogPrintf("%s: parameter interaction: -liquidityprovider=%d -> setting -anonsendamount=%d\n", __func__, nLiqProvTmp, MAX_ANONSEND_AMOUNT);
        ForceSetArg("-anonsendmultisession", "0");
        LogPrintf("%s: parameter interaction: -liquidityprovider=%d -> setting -anonsendmultisession=0\n", __func__, nLiqProvTmp);
    }
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("MarteX Core version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single MarteX Core process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    // ********************************************************* Step 11a: setup AnonSend
    fMasternodeMode = GetBoolArg("-masternode", false);
    // TODO: masternode should have no wallet

    //lite mode disables all MarteX-specific functionality
    fLiteMode = GetBoolArg("-litemode", false);

    if(fLiteMode) {
        InitWarning(_("You are starting in lite mode, all MarteX-specific functionality is disabled."));
    }

    if((!fLiteMode && fTxIndex == false)
       && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/martexcoin/martexcoin/pull/1817 and https://github.com/martexcoin/martexcoin/pull/1743
    ...
    anonSendClient.nLiquidityProvider = std::min(std::max((int)GetArg("-liquidityprovider", DEFAULT_ANONSEND_LIQUIDITY), MIN_ANONSEND_LIQUIDITY), MAX_ANONSEND_LIQUIDITY);
    int nMaxRounds = MAX_ANONSEND_ROUNDS;
    if(anonSendClient.nLiquidityProvider) {
        // special case for liquidity providers only, normal clients should use default value
        anonSendClient.SetMinBlocksToWait(anonSendClient.nLiquidityProvider * 15);
        nMaxRounds = std::numeric_limits<int>::max();
    }

    anonSendClient.fEnableAnonSend = GetBoolArg("-enableanonsend", false);
    anonSendClient.fAnonSendMultiSession = GetBoolArg("-anonsendmultisession", DEFAULT_ANONSEND_MULTISESSION);
    anonSendClient.nAnonSendRounds = std::min(std::max((int)GetArg("-anonsendrounds", DEFAULT_ANONSEND_ROUNDS), MIN_ANONSEND_ROUNDS), nMaxRounds);
    anonSendClient.nAnonSendAmount = std::min(std::max((int)GetArg("-anonsendamount", DEFAULT_ANONSEND_AMOUNT), MIN_ANONSEND_AMOUNT), MAX_ANONSEND_AMOUNT);
#endif // ENABLE_WALLET

    fEnableFastSend = GetBoolArg("-enablefastsend", 1);
    nFastSendDepth = GetArg("-fastsenddepth", DEFAULT_FASTSEND_DEPTH);
    nFastSendDepth = std::min(std::max(nFastSendDepth, MIN_FASTSEND_DEPTH), MAX_FASTSEND_DEPTH);

    LogPrintf("fLiteMode %d\n", fLiteMode);
    LogPrintf("nFastSendDepth %d\n", nFastSendDepth);
#ifdef ENABLE_WALLET
    LogPrintf("AnonSend liquidityprovider: %d\n", anonSendClient.nLiquidityProvider);
    LogPrintf("AnonSend rounds: %d\n", anonSendClient.nAnonSendRounds);
    LogPrintf("AnonSend amount: %d\n", anonSendClient.nAnonSendAmount);
#endif // ENABLE_WALLET

    CAnonSend::InitStandardDenominations();
    ...
    // ********************************************************* Step 11c: update block tip in MarteX modules
    ...
    // ********************************************************* Step 11d: start martex-ps-<smth> threads

    threadGroup.create_thread(boost::bind(&ThreadCheckAnonSend, boost::ref(*g_connman)));
    if (fMasternodeMode)
        threadGroup.create_thread(boost::bind(&ThreadCheckAnonSendServer, boost::ref(*g_connman)));
#ifdef ENABLE_WALLET
    else
        threadGroup.create_thread(boost::bind(&ThreadCheckAnonSendClient, boost::ref(*g_connman)));
#endif // ENABLE_WALLET
#ifdef ENABLE_WALLET
