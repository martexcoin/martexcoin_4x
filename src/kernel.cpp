...
// Stake ModifierV2 (hash modifier of proof-of-stake):
// The purpose of stake modifier is to prevent a txout (coin) owner from
// computing future proof-of-stake generated by this txout at the time
// of transaction confirmation. To meet kernel protocol, the txout
// must hash with a future stake modifier to generate the proof.
...
bool OldCheckStakeKernelHash(const CBlockIndex* pindexPrev, unsigned int nBits, unsigned int nTimeBlockFrom, const CTransactionRef& txPrev, const COutPoint& prevout, unsigned int nTimeTx, uint256& hashProofOfStake, uint256& targetProofOfStake)
{
    auto nStakeMinAge = Params().GetConsensus().nStakeMinAge;
...
bool OldCheckProofOfStake(CValidationState &state, const CBlockIndex* pindexPrev, const CTransaction& tx, unsigned int nBits, uint256& hashProofOfStake, uint256& targetProofOfStake)
{
    ...
    if (!pcoinsTip->GetCoin(txin.prevout, coin) || coin.IsSpent()) {
        ...
        uint256 hashBlock; // block containing stake kernel, GetTransaction should only fill the header.
        ...
        if (!OldCheckStakeKernelHash(pindexPrev, nBits, pindexPrev->GetBlockTime(), txPrev, txin.prevout, tx.nTime, hashProofOfStake, targetProofOfStake))
...
bool CheckAge(const CBlockIndex *pindexTip, const uint256 &hashKernelBlock, int &nDepth)
{
    ...
    int nRequiredDepth = std::min(nStakeMinConfirmations-1, (int)(pindexTip->nHeight / 2));
...
bool CheckKernel(CBlockIndex* pindexPrev, unsigned int nBits, int64_t nTime, const COutPoint& prevout, int64_t* pBlockTime)
{
    ...
    {
        if (txPrev->nTime + nStakeMinAge > nTime)
           return false; // only count coins meeting min age requirement
    }

    return OldCheckStakeKernelHash(pindexPrev, nBits, *pBlockTime, txPrev, prevout, nTime, hashProofOfStake, targetProofOfStake);
}
