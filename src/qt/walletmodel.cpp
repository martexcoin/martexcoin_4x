...
bool WalletModel::validateAddress(const QString &address)
{
    ...
    CMarteXAddress addressParsed(sAddr);
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
...

    {
        ...
        // reject martex fee
        unsigned int martexfee = (transaction.getTransactionSize() == 0 ? nAddresses : transaction.getTransactionSize());
        martexfee = ((MIN_RELAY_TX_FEE * martexfee) * 10000);
        if (nFeeRequired > martexfee){
            LogPrintf("nFeeRequired: %d -- MartexFee: %d\n", nFeeRequired, martexfee);
            return MartexFee;
        }
    }
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            CScript scriptPubKey;
            scriptPubKey.SetDestination(CMarteXAddress(sAddr).Get());
            vecSend.push_back(make_pair(scriptPubKey, rcp.amount));
...
{
    // MarteX context; old object no longer relocks wallet
    *this = rhs;
    rhs.relock = false;
}
