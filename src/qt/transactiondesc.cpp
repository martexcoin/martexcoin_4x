...
QString TransactionDesc::FormatTxStatus(const CWalletTx& wtx)
{
    ...
    else
    {
        ...
        if(!fastsend.HasTxLockRequest(wtx.GetHash())) return strTxStatus; // regular tx

        int nSignatures = fastsend.GetTransactionLockSignatures(wtx.GetHash());
        int nSignaturesMax = CTxLockRequest(wtx).GetMaxSignatures();
        // FastSend
        strTxStatus += " (";
        if(fastsend.IsLockedFastSendTransaction(wtx.GetHash())) {
            strTxStatus += tr("verified via FastSend");
        } else if(!fastsend.IsTxLockCandidateTimedOut(wtx.GetHash())) {
            strTxStatus += tr("FastSend verification in progress - %1 of %2 signatures").arg(nSignatures).arg(nSignaturesMax);
        } else {
            strTxStatus += tr("FastSend verification failed");
        }
        strTxStatus += ")";
