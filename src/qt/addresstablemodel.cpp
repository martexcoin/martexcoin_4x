...
bool AddressTableModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case Label:
            ...
            {
                wallet->SetAddressBookName(CMarteXAddress(strTemp).Get(), value.toString().toStdString());
            }
            break;
        case Address:
            ...
            if(CMarteXAddress(rec->address.toStdString()) == CMarteXAddress(value.toString().toStdString()))
            {
                editStatus = NO_CHANGES;
                return false;
            }
            ...
            else if(wallet->mapAddressBook.count(CMarteXAddress(value.toString().toStdString()).Get()))
            {
            ...
            {
                {
                    LOCK(wallet->cs_wallet);
                    // Remove old entry
                    wallet->DelAddressBookName(CMarteXAddress(rec->address.toStdString()).Get());
                    // Add new entry with new address
                    wallet->SetAddressBookName(CMarteXAddress(value.toString().toStdString()).Get(), rec->label.toStdString());
                }
            }
            break;
        }
    ...
    if(type == Send)
    {
        ...
        } else
        {
            ...
            // Check for duplicate addresses
            {
                ...
                wallet->SetAddressBookName(CMarteXAddress(strAddress).Get(), strLabel);
            }
        }
    }
...
{
    {
        ...
        {
            CMarteXAddress address_parsed(sAddr);
