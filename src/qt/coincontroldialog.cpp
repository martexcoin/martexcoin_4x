...
bool CCoinControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    ...
    if (column == CoinControlDialog::COLUMN_AMOUNT || column == CoinControlDialog::COLUMN_DATE || column == CoinControlDialog::COLUMN_CONFIRMATIONS || column == CoinControlDialog::COLUMN_ANONSEND_ROUNDS)
    ...
    platformStyle(_platformStyle)
{
    ...
    ui->treeWidget->setColumnWidth(COLUMN_ANONSEND_ROUNDS, 88);
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    BOOST_FOREACH(const COutput& out, vOutputs) {
        ...
        // Add inputs to calculate FastSend Fee later
        if(coinControl->fUseFastSend)
    ...
    if (nQuantity > 0)
    {
        ...
        // FastSend Fee
        if (coinControl->fUseFastSend) nPayFee = std::max(nPayFee, CTxLockRequest(txDummy).GetMinFee());
        ...
        if (nPayAmount > 0)
        {
            ...
            // AnonSend Fee = overpay
            if(coinControl->fUseAnonSend && nChange > 0)
            {
                nPayFee += nChange;
                nChange = 0;
            }
